...
class batch_lstm(nn.Module):
    ...
    def forward(self,inp,slens):
        ...
        # feed through lstm, ignoring the hidden states
        tmp_otp,_ = self.lstm(pckd_inp,(h0,c0))

        # repad "PackedSequence" object so it can actually be used
        #   - ignore the sequence lengths, we have those already
        otp,_ = nn.utils.rnn.pad_packed_sequence(tmp_otp,padding_value=float('nan'),
                                                batch_first=True)

        # run through fully connected linear layer
        otp = self.fc(otp)
